...
# 跑单一文件 / 单一方法
php artisan test tests/Feature/Api/DepositApplyTest.php
php artisan test --filter=deposit_apply_with_valid_signature_returns_success
...
4. 跑 `php artisan test --env=testing`
...
```bash
php artisan test tests/Feature/Admin/ScreenSmokeTest.php
```
...
php artisan i18n:import zh-CN --overwrite

# 导出 DB 到 lang/*.json
php artisan i18n:export
php artisan i18n:export zh-CN

# 清缓存
php artisan i18n:cache:clear
```
...
- **新增 UI 文案后**，提交前跑一次 `php artisan i18n:scan`，确保 DB 中各语言都已登记对应 key
