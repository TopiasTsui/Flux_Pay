...
class AgentWalletListScreen extends Screen
{
    ...
    public function query(Request $request): iterable
    {
        ...
        $query = AgentWalletRecord::with('agent')
        ...
        return [
            ...
            Layout::table('records', [
                ...
                TD::make('type_code', __('Type'))
                    ->render(fn (AgentWalletRecord $r) => WalletOperationType::tryFrom($r->type_code)?->label() ?? $r->type_code),
                ...
                TD::make('created_at', __('Created'))->sort()
                    ->render(fn (AgentWalletRecord $r) => $r->created_at?->format('Y-m-d H:i:s')),
            ]),
        ];
    }
